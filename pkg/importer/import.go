...
func ImportBundle(ctx context.Context, b bundle.Bundle, cfg *rest.Config) error {
...

	{
		return afero.Walk(b, b.Layout().ClusterResources(), func(path string, info fs.FileInfo, err error) error {
			...
			if info.IsDir() && strings.Contains(path, "auth-cani-list") {
				return fs.SkipDir
			}
...
func importObject(ctx context.Context, cl *dynamic.DynamicClient, gvr schema.GroupVersionResource, o runtime.Object, includeStatus bool) error {
